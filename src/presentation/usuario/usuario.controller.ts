...
export class UsuarioController {
	...
	upsertRoles = async (req: Request, res: Response) => {
		const upsertDTO = req.body as UpsertUsuarioRolDTO;
		try {
			const { id, message, result } = await this.usuarioRepository.upsertRoles(upsertDTO);
			res.json({
				message,
				data: id,
				result,
